...
## Infection Chain
1. **Initial Vector:** A PowerShell script (`hack2.ps1`) uses `Invoke-WebRequest` to download an image file (`my_newest_ll.png`).
...
5. **Final Stage (Infostealer):** `my_s_real.bin` is a heavily obfuscated 312 KB `.NET` assembly. At runtime, it utilizes an embedded substitution cipher to decode constants and targets sensitive applications (Chrome App-Bound Encryption markers, Steam configuration, etc.).

## Contents
* `docs/audit.md`: The complete, step-by-step reverse engineering audit and findings.
* `docs/IoCs.md`: Indicators of Compromise (IPs, URLs) for threat hunting.
* `docs/full_technical_report.md`: Detailed static capability report.
